...
class TemporalEncoder(nn.Module):
    ...
    def forward(self, x, time_steps=8):
        # 如果输入已经是 [batch, time, features]
        if len(x.shape) == 3:
            # 直接通过线性层，Linear 会作用在最后一个维度上
            # 结果形状依然是 [batch, time, input_size]
            encoded = self.encoder(x)
            encoded = self.dropout(encoded)
            # 如果时间步已经够了，直接返回；如果不够，再考虑 repeat
            if encoded.size(1) == time_steps:
                return encoded
            else:
                # 这种场景通常发生在输入是单步，需要扩展成多步时
                return encoded.repeat(1, time_steps // encoded.size(1), 1)

        # 如果输入是 [batch, features]
        encoded = self.encoder(x)
        encoded = self.dropout(encoded)
        temporal = encoded.unsqueeze(1).repeat(1, time_steps, 1)
        return temporal

...
class SNN(nn.Module):
    ...
    def __init__(self, input_size, hidden_size=128, num_classes=3, time_steps=8):
        ...
        self.encoder = SpikeEncoder[self._snn_backend]["conv"](time_steps)
    ...
    def forward_one(self, x):
        ...
        # 换编码器
        x = self.encoder(x)  # B, H, C, L
        print(f"DEBUG: Encoder output shape: {x.shape}")
        if x.dim() == 4:
            # 将 [128, 8, 2, 250] 转换为 [128, 8, 500]
            batch_size, t_steps, channels, seq_len = x.shape
            x = x.reshape(batch_size, t_steps, channels * seq_len)

            # 3. 时序处理
            # 此时 x 的形状是 [128, 8, 500]，符合 input_size=500 的要求
